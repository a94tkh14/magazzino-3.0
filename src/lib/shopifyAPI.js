...
// FUNZIONE SEMPLICE per testare Shopify da zero
export const fetchShopifyOrders = async (limit = 50, onProgress) => {
  try {
    ...
    // Test 1: Endpoint base senza filtri
    console.log(`\n📋 TEST 1: Endpoint base senza filtri`);
    const requestBody = {
      shopDomain: credentials.shopDomain,
    ...
    if (data.success && data.data && data.data.orders) {
      ...
      console.log(`✅ SUCCESSO! Trovati ${orders.length} ordini`);
      console.log(`📊 Primi 3 ordini:`, orders.slice(0, 3).map(o => ({
        id: o.id,
        name: o.name,
        status: o.status,
      ...
      if (onProgress) onProgress(orders.length, 1);
      return orders;
    } else {
      console.log(`⚠️ Response non valida:`, data);
      return [];
    }
...
// Funzione per recuperare ordini APERTI (da evadere) da Shopify
export const fetchShopifyOpenOrders = async (limit = 250, onProgress, daysBack = null) => {
  try {
    const credentials = getShopifyCredentials();
    let allOrders = [];
    let pageInfo = null;
    ...
    console.log(`🔄 Inizio scaricamento ordini APERTI Shopify (limite: ${limit})`);

    while (keepGoing) {
      console.log(`📄 Scaricamento pagina ${page} ordini aperti...`);
      
      // Prepara il body della richiesta per ordini APERTI
      const requestBody = {
        shopDomain: credentials.shopDomain,
        accessToken: credentials.accessToken,
        apiVersion: credentials.apiVersion,
        testType: 'open_orders',
        limit: limit
      };
      ...
      console.log(`🔍 DEBUG - Request body per ordini aperti:`, JSON.stringify(requestBody, null, 2));
      ...
      if (!response.ok) {
        ...
        throw new Error(`Errore API Shopify ordini aperti: ${errorData.error || response.statusText}`);
      }
      ...
      if (data.success && data.data.orders && data.data.orders.length > 0) {
        ...
        console.log(`[SYNC APERTI] Scaricati ${allOrders.length} ordini aperti totali dopo pagina ${page}`);
        console.log(`📊 Ordini aperti in questa pagina: ${data.data.orders.length}/${limit}`);
        console.log(`📈 Totale ordini aperti scaricati finora: ${allOrders.length}`);
        ...
        if (data.data.orders.length < limit) {
          ...
          if (linkHeader && linkHeader.includes('rel="next"')) {
            ...
            if (data.data.orders.length > 0 && data.data.orders.length === limit) {
              ...
              console.log(`🔍 Questo potrebbe significare che abbiamo raggiunto tutti gli ordini aperti disponibili`);
              keepGoing = false;
            }
          }
        }
      } else {
        console.log(`⚠️ Pagina ${page} non ha ordini aperti o errore`);
        if (data.error) {
      ...
      if (page > 200) {
        ...
        console.log(`🔍 Ordini aperti totali scaricati: ${allOrders.length}`);
    ...
    console.log(`🎉 Scaricamento ordini aperti completato! Totale ordini: ${allOrders.length}`);
    console.log(`📊 Limite richiesto: ${limit}`);
